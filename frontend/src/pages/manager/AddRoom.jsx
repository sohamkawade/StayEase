...
const AddRoom = () => {
      ...
      if (!resolvedHotelId && user) {
        ...
        const match = hotels.find((h) => {
          ...
          const matchByPhone =
            phone &&
            m.contactNumber &&
            String(m.contactNumber) === String(phone);
  ...
  useEffect(() => {
    const arr =
      imagesWatch instanceof FileList
        ? Array.from(imagesWatch)
        : imagesWatch || [];
    setLocalFiles(arr);
  }, [imagesWatch]);
  ...
  const onSubmit = async (data) => {
    if (!hotelId) {
      setError("root", {
        type: "manual",
        message:
          "Hotel ID not found. Please ensure you are logged in as a manager.",
      });
      return;
    }
    ...
    setSubmitting(true);
    const statusEnum =
      (data.status || "").toString().toLowerCase() === "available"
        ? "AVAILABLE"
    ...
    const files =
      data.roomImages instanceof FileList
        ? Array.from(data.roomImages)
        : data.roomImages || [];
    if (files.length < 1) {
  ...
  const handleImagesChange = (e) => {
    ...
    if (incoming.length > MAX_IMAGES || merged.length > MAX_IMAGES) {
      setImageError(
        `Maximum ${MAX_IMAGES} images allowed. Extra files ignored.`
      );
      setError("roomImages", {
        type: "max",
        message: `Max ${MAX_IMAGES} images`,
      });
      ...
      <div className="flex flex-col w-full bg-gray-200 text-gray-800 p-4 sm:p-6 md:p-10 lg:p-6 rounded-2xl">
        <div className="mb-4 sm:mb-6">
          ...
          <p className="text-sm sm:text-base text-gray-600">
            Create a new room for this hotel
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow p-4 sm:p-6">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
              <div>
                <label className="font-medium text-black mb-1 block text-sm sm:text-base">
                  Room Number
                </label>
                <input
                  type="text"
            ...
            <div>
              <label className="font-medium text-black mb-2 block text-sm sm:text-base">
                Amenities
              </label>
              <div className="flex flex-wrap gap-2 sm:gap-3">
                {[
                  ...
                  <label
                    key={a}
                    className="inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-gray-50 rounded-xl border border-gray-300 cursor-pointer select-none"
                  >
                    <input
                      type="checkbox"
                      value={a}
            ...
            <div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                ...
                <span className="text-xs text-gray-500">
                  {(imagesWatch && imagesWatch.length) || 0}/{MAX_IMAGES}{" "}
                  selected
                </span>
              </div>
              {imageError && (
                ...
                <div className="mt-3 overflow-x-auto">
                  <div className="flex gap-2 sm:gap-3 whitespace-nowrap">
                    {imagePreviews.map((img, idx) => (
                      <div
                        key={`${img.name}-${idx}`}
                        className="relative w-20 h-20 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-gray-200 border border-gray-300 flex-none"
                      >
                        <img
                          src={img.url}
                          alt={img.name}
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              <button
                type="submit"
                disabled={submitting}
                className="flex items-center justify-center gap-2 px-4 sm:px-6 py-2 rounded-xl font-semibold bg-black text-white shadow hover:bg-gray-900 hover:scale-95 transition-transform duration-300 text-xs sm:text-sm w-full sm:w-auto"
              >
              ...
              <button
                type="button"
                onClick={() => reset()}
                className="px-4 sm:px-6 py-2 rounded-xl text-xs sm:text-sm bg-white text-gray-700 border border-gray-300 hover:scale-95 transition w-full sm:w-auto"
              >
                Reset
              </button>
            </div>
